...
describe('TreasureMarketplace', function () {
    ...
    beforeEach(async function () {
        ...
        const proxy = await OptimizedTransparentUpgradeableProxy.deploy(await marketplaceImpl.getAddress(), admin, data)
    ...
    describe('init', function () {
        it('initialize()', async function () {
            await expect(marketplace.initialize(100, feeRecipient, await magicToken.getAddress())).to.be.revertedWith("Initializable: contract is already initialized");
        });
        ...
        it('setFee()', async function () {
            ...
            await expect(marketplace.setFee(tooHighFee, newFeeWithCollectionOwner)).to.be.revertedWith("TreasureMarketplace: max fee");
        ...
        it('setCollectionOwnerFee()', async function () {
            ...
            await (await marketplace.setTokenApprovalStatus(await nft.getAddress(), TOKEN_APPROVAL_STATUS_ERC_721_APPROVED, await magicToken.getAddress())).wait();
            ...
            await expect(marketplace
                ...
                .setCollectionOwnerFee(await nft.getAddress(), collectionOwnerFee))
            ...
            expect((await marketplace.collectionToCollectionOwnerFee(await nft.getAddress())).recipient)
                .to.be.equal(seller);
            expect((await marketplace.collectionToCollectionOwnerFee(await nft.getAddress())).fee)
                .to.be.equal(500);
        });
        ...
        it('approve token', async function () {
            expect(await marketplace.tokenApprovals(await nft.getAddress())).to.equal(TOKEN_APPROVAL_STATUS_NOT_APPROVED);
            await marketplace.setTokenApprovalStatus(await nft.getAddress(), TOKEN_APPROVAL_STATUS_ERC_721_APPROVED, await magicToken.getAddress());
            expect(await marketplace.tokenApprovals(await nft.getAddress())).to.equal(TOKEN_APPROVAL_STATUS_ERC_721_APPROVED);
            // Allow to approve twice
            await marketplace.setTokenApprovalStatus(await nft.getAddress(), TOKEN_APPROVAL_STATUS_ERC_721_APPROVED, await magicToken.getAddress());
            expect(await marketplace.tokenApprovals(await nft.getAddress())).to.equal(TOKEN_APPROVAL_STATUS_ERC_721_APPROVED);

            await expect(
                marketplace.setTokenApprovalStatus(await nft.getAddress(), TOKEN_APPROVAL_STATUS_ERC_1155_APPROVED, await magicToken.getAddress())
            ).to.be.revertedWith("TreasureMarketplace: not an ERC1155 contract");
        });

        it('approve token with weth', async function () {
            expect(await marketplace.tokenApprovals(await nft.getAddress())).to.equal(TOKEN_APPROVAL_STATUS_NOT_APPROVED);
            await marketplace.setTokenApprovalStatus(await nft.getAddress(), TOKEN_APPROVAL_STATUS_ERC_721_APPROVED, await weth.getAddress());
            expect(await marketplace.tokenApprovals(await nft.getAddress())).to.equal(TOKEN_APPROVAL_STATUS_ERC_721_APPROVED);

            expect(await marketplace.getPaymentTokenForCollection(await nft.getAddress()))
                .to.equal(await weth.getAddress());
        });

        it('unapprove token', async function () {
            await marketplace.setTokenApprovalStatus(await nft.getAddress(), TOKEN_APPROVAL_STATUS_ERC_721_APPROVED, await magicToken.getAddress());
            expect(await marketplace.tokenApprovals(await nft.getAddress())).to.equal(TOKEN_APPROVAL_STATUS_ERC_721_APPROVED);
            await marketplace.setTokenApprovalStatus(await nft.getAddress(), TOKEN_APPROVAL_STATUS_NOT_APPROVED, await magicToken.getAddress());
            expect(await marketplace.tokenApprovals(await nft.getAddress())).to.equal(TOKEN_APPROVAL_STATUS_NOT_APPROVED);
    ...
    describe('ERC721', function () {
        it('createOrUpdateListings()', async function () {
            ...
            await expect(marketplace.connect(sellerSigner).createOrUpdateListings([ListingsToCreate.VALID_1])).to.be.revertedWith("TreasureMarketplace: token is not approved for trading");
            await marketplace.setTokenApprovalStatus(await nft.getAddress(), TOKEN_APPROVAL_STATUS_ERC_721_APPROVED, await magicToken.getAddress());
            ...
            await expect(marketplace.connect(buyerSigner).createOrUpdateListings([ListingsToCreate.VALID_1])).to.be.revertedWith("TreasureMarketplace: not owning item")
            await marketplace.pause();
            ...
            const listing = await marketplace.listings(await nft.getAddress(), ListingsToCreate.VALID_1.tokenId, seller)
            ...
            let listing = await marketplace.listings(await nft.getAddress(), ListingsToUpdate.INCREASE_PRICE.tokenId, seller)
            ...
            await Promise.all([
                    ...
                    const listing = await marketplace.listings(await nft.getAddress(), expectedListing.tokenId, seller)
    ...
    describe('ERC1155', function () {
        it('createOrUpdateListings()', async function () {
            ...
            await expect(marketplace.connect(sellerSigner).createOrUpdateListings([ListingsToCreate.VALID_1])).to.be.revertedWith("TreasureMarketplace: token is not approved for trading");
            await marketplace.setTokenApprovalStatus(await erc1155.getAddress(), TOKEN_APPROVAL_STATUS_ERC_1155_APPROVED, await magicToken.getAddress());
            await expect(marketplace.connect(sellerSigner).createOrUpdateListings([ListingsToCreate.VALID_1, ListingsToCreate.INVALID_MIN_PRICE])).to.be.revertedWith("TreasureMarketplace: below min price")
            await expect(marketplace.connect(sellerSigner).createOrUpdateListings([ListingsToCreate.VALID_1, ListingsToCreate.INVALID_EXPIRATION])).to.be.revertedWith("TreasureMarketplace: invalid expiration time")
            ...
            const createdListings = [ListingsToCreate.VALID_1, ListingsToCreate.VALID_2, ListingsToCreate.VALID_3]
            await Promise.all(createdListings.map(async (createdListing) => {
            const listing = await marketplace.listings(await erc1155.getAddress(), createdListing.tokenId, seller)
                expect(listing.quantity).to.be.equal(createdListing.quantity);
                expect(listing.pricePerItem).to.be.equal(createdListing.pricePerItem);
                expect(listing.expirationTime).to.be.equal(createdListing.expirationTime);
            }))
            ...
            let listing = await marketplace.listings(await erc1155.getAddress(), ListingsToUpdate.INCREASE_PRICE.tokenId, seller)
            expect(listing.pricePerItem).to.be.equal(listing.pricePerItem);
            ...
            await Promise.all([
                ...
                const listing = await marketplace.listings(await erc1155.getAddress(), expectedListing.tokenId, seller)
                expect(listing.quantity).to.be.equal(expectedListing.quantity);
                expect(listing.pricePerItem).to.be.equal(expectedListing.pricePerItem);
                expect(listing.expirationTime).to.be.equal(expectedListing.expirationTime);
            }))
        ...
        describe('with NFT minted', function () {
            ...
            describe('expirationTime', function () {
                ...
                it('success', async function () {
                    ...
                    await magicToken.connect(buyerSigner).approve(await marketplace.getAddress(), pricePerItem * BigInt(quantity));
                    expect(await magicToken.balanceOf(await marketplace.getAddress())).to.be.equal(0);
                    expect(await magicToken.balanceOf(seller)).to.be.equal(0);
            ...
            describe('with listing', function () {
                ...
                describe('buyItem()', function () {
                    ...
                    it('partial', async function () {
                        ...
                        await magicToken.connect(buyerSigner).approve(await marketplace.getAddress(), pricePerItem * BigInt(buyQuantity));
                        expect(await magicToken.balanceOf(await marketplace.getAddress())).to.be.equal(0);
                        expect(await magicToken.balanceOf(seller)).to.be.equal(0);
    ...
    it('Should be able to create a valid 721/1155 token bid', async function () {
        ...
        await(await marketplace.setTokenApprovalStatus(await nft.getAddress(), TOKEN_APPROVAL_STATUS_ERC_721_APPROVED, await magicToken.getAddress())).wait();
        await(await marketplace.setTokenApprovalStatus(await erc1155.getAddress(), TOKEN_APPROVAL_STATUS_ERC_1155_APPROVED, await magicToken.getAddress())).wait();
        ...
        await(await magicToken
            .connect(buyerSigner)
            .approve(await marketplace.getAddress(), pricePerItem))
            .wait();
    ...
    it('Should be able to create a valid 721 collection bid', async function () {
        ...
        await(await magicToken
            .connect(buyerSigner)
            .approve(await marketplace.getAddress(), pricePerItem * 2n))
            .wait();
    ...
    it('Should be able to accept a collection level bid', async function () {
        ...
        await(await magicToken
            .connect(buyerSigner)
            .approve(await marketplace.getAddress(), pricePerItem * 3n))
            .wait();
